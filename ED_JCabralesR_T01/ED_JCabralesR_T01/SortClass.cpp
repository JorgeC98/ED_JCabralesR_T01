...
void SortClass::quickSort(int a[], int inicio, int fin) {  //REVISAR: Agregamos metodo intercambio?
	int i = inicio;
	int j = fin;
...
void SortClass::mergeSort(int arr[], int inicio, int fin) {
	if (inicio < fin) {
...
void SortClass::merge(int arr[], int inicio, int mitad, int fin) {
	int* helper = new int[sizeof(arr)];

	for (int i = inicio; i <= fin; i++) { //Copiar el arreglo en nuestro auxiliar
		helper[i] = arr[i];
	}
